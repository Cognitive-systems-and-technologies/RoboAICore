...
#ifdef __NVCC__
__global__ void Tensor_FillKernel(float *w, float v) 
{
...
void Tensor_FillGPU(Tensor *v, float c) 
{
	...
	if (v->n < threadsPerBlockX) threadsPerBlockX = 1;
	dim3 gridDim(ceil(v->n / (float)threadsPerBlockX), 1, 1);
	dim3 blockDim(threadsPerBlockX, 1, 1);
	Tensor_FillKernel KERNEL_CALL(gridDim, blockDim) (v->w, c);
...
void Tensor_FillArrayGPU(float* v, int n, float c)
{
	...
	dim3 blockDim(threadsPerBlockX, 1, 1);
	Tensor_FillKernel KERNEL_CALL(gridDim, blockDim) (v, c);
	cudaDeviceSynchronize();
}
...
__global__ void setup_rng_kernel(void* state)
{
...
void Tensor_Xavier_RandGPU(float *w, int n)
{
	...
	setup_rng_kernel KERNEL_CALL(n, 1) (devStates);
	cudaDeviceSynchronize();
